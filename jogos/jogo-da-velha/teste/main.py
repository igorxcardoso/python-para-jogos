...
class TicTacToe:
    ...
    def draw_x_and_o (self, window):
        ...
        tic_tac_toe.draw_win_line(window)
...
while True:
    ...
    button_new_game = w.botao((675, 225), (200, 75), w.colors['green'], 'Novo jogo', w.mouse)
    button_reset = w.botao((675, 325), (200, 75), w.colors['orange'], 'Reiniciar', w.mouse)
    botao_de_sair = w.botao((695, 600), (150, 75), w.colors['red'], 'Sair', w.mouse)

    if botao_de_sair == 'Sair':
        quit()
    if button_new_game == 'Novo jogo':
        tic_tac_toe.new_game()
    if button_reset == 'Reiniciar':
